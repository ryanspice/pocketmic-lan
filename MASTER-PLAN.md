...
8. **The desktop-control review found release-relevant defects.** Its source review and runtime checks cover buffer control, monitor routing, DSP strength, Windows default-microphone switching/restoration, and exporting diagnostics during an active session. D1 now has explicit automatic/manual ownership, valid fallback bounds, a separate effective-target readout, and serialized mode settings. D2-D5 remain open: live monitor changes, Strength after phone Custom, selected-cable-only microphone switching with per-role rollback/restoration, and report export during an active session. D1's desktop build, 161 core tests, and latest branch CI pass; interactive UI/audio behavior remains unverified.
