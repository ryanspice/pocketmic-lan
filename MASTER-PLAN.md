...
The latest pushed source commit is `1d7f5c0540b3029a83cc9025d9da66a15cb8405b` (`fix(windows): route only to the selected microphone cable`). Its push-triggered primary CI, iOS, and macOS workflows passed; the associated pull-request CI, iOS, macOS, and Lighthouse runs also passed. The current working tree additionally contains D5 active-session report export changes with 173 local Windows tests and a clean desktop build; hosted CI for that change is pending. See the dated receipts in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). These are development-branch checks, not a frozen release-candidate rerun. Recheck the live workflows before tagging.
