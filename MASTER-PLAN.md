...
| Windows, Android, web CI | Green on current branch commit `ce82d04b681418ffd88f48ae69f3882c8a861b38`; Android debug build/lint/JVM tests, Windows build/tests, and website release validation passed. | [Push CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094058898), [PR CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094061959), and [PR Lighthouse](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094061876). Build artifact metadata is recorded in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). |
| iOS | Simulator app builds on `ce82d04`; package contains 38-string `en-US`, `ckb`, and `ku-Latn` UI catalogs plus localized microphone and local-network purpose strings. The app remains an experimental PCM v1 publisher. | [Push iOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094058888) and [PR iOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094061892) passed. Simulator artifact metadata is in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). No physical iPhone-to-receiver acceptance is recorded. |
| macOS | Hosted CI built the unsigned universal app and CoreAudio virtual-microphone driver, passed protocol tests and validated the exact uploaded archives. The validator checks app/driver bundle IDs, 0.1.6 version metadata, `en-CA`, both CPU architectures, and tester install/recovery files. | [Push macOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094058884) and [PR macOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094061875) passed. Artifact digest and per-ZIP hashes are recorded in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). This remains build/protocol-test/package evidence, not driver installation or live routing acceptance. |
...
| Locale work | Android has `en-CA`, `en-US`, and preliminary `ckb`/`kmr` catalogs. Apple bundles verify the `en-CA` default, 38-string UI catalogs, distinct Sorani/Kurmanji resources, and localized system purpose strings. The marketing HTML source is now explicitly tagged `en-CA`; the text is still English-only and no site locale selector or translated catalog exists. | Apple en-US copy needs regional adaptation/review; Kurdish drafts need fluent and script/layout review; Windows localization is unimplemented and site translations remain open. Do not claim unreviewed locales as supported. See [`localization/README.md`](localization/README.md), [`localization/catalog-status.json`](localization/catalog-status.json), [`localization/apple/catalog-status.json`](localization/apple/catalog-status.json), and [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). |
| Website release-source truth | Local `dev/v3` release manifest and Android/Windows download pages match the published v0.1.5 assets, SHA-256 values, and sizes; a Node validation is included in web CI. Commit `d676fae` removes the false claim that PM-LAN is a bundled Windows virtual cable: the documented route requires separately installed VB-CABLE, with `CABLE Input` selected as the receiver output and `CABLE Output` selected in PocketMic. | Source correction and local validation passed. Hosted CI on `d676fae` [passed](https://github.com/ryanspice/pocketmic-lan/actions/runs/36093465092), and the current branch web check [passed](https://github.com/ryanspice/pocketmic-lan/actions/runs/36094058898). Deployment, direct live-page verification, and physical end-to-end routing acceptance remain separate evidence gates. |
