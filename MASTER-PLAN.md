...
| Windows, Android, and application build CI | Windows build/full tests/package, Android debug APK/lint/JVM tests, iOS simulator build, and macOS build/package passed on prior application candidate `76d7ce72905b59be89045e3d98cb05b7cc7bf91e`. That candidate predates integration of the selected marketing site. | Prior [CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980901), [Lighthouse](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980930), [iOS](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980929), and [macOS](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980945) runs remain useful for application evidence but do not validate the selected site; the integrated candidate needs fresh hosted runs. |
| Windows release package | Hosted Windows CI builds the x64 receiver with pinned Opus 1.5.2, runs the full test suite including native Opus smoke coverage, validates third-party notices, and uploads a self-contained ZIP. | Current [manual-test preview](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980901/artifacts/10853728561) is a GitHub artifact archive of 68,185,440 bytes with digest `sha256:62a0fd81da9588fdf13e4615ef6ceadbdfa1b3faba0ada56c65d865c92b18651`; this digest covers the outer workflow artifact, not the ZIP extracted from it. It is an unsigned development preview, not a tagged release or physical Windows acceptance. |
| iOS | Simulator app builds on the current candidate; CI also runs shared PCM v1 packet-vector tests. Package contains `en-CA`, `en-US`, `ckb`, and `ku-Latn` UI catalogs plus localized microphone and local-network purpose strings. The app remains an experimental PCM v1 publisher. | [Latest iOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980929) passed. Simulator artifact metadata is in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). No physical iPhone-to-receiver acceptance is recorded; the branch artifact is simulator-only. |
| macOS | Hosted CI built the unsigned universal app and CoreAudio virtual-microphone driver, passed protocol tests and validated the exact uploaded archives. The validator checks app/driver bundle IDs, 0.1.6 version metadata, `en-CA`, both CPU architectures, and tester install/recovery files. | [Latest macOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36112980945) passed. Artifact metadata is in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). This remains build/protocol-test/package evidence, not driver installation or live routing acceptance. |
...
| Website release-source truth | The selected cream-and-green homepage is now integrated in canonical `dev/v3/`; its 16-route manifest, sitemap, local CTAs, current v0.1.5 download facts, privacy disclosures, and `en-CA` default are checked by `scripts/validate-site-release.mjs`. CI and Lighthouse target the same `dev/v3/` payload. | Local validator and browser review pass; the refreshed hosted CI run for this payload is pending. Site copy remains English-only except the labelled US spelling preview. Kurdish/site translations, hands-on visual/accessibility acceptance, deployment, live-page verification, and physical product routing remain separate gates. |
