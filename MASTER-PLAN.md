...
| Windows, Android, web, and application build CI | On candidate commit `58366868b4fb6c511ef40caa385b084ea76483f2`, Windows build/full tests/package, Android debug APK/lint/`testDebugUnitTest`, and canonical site validation passed. iOS simulator/protocol tests, macOS build/package, and Lighthouse also passed. | [CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161097), [Lighthouse](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161065), [iOS](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161174), and [macOS](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161353) passed. Artifacts are linked in [`MANUAL-TESTING-0.1.6.md`](MANUAL-TESTING-0.1.6.md) and recorded in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). |
| Windows release package | Hosted Windows CI builds the x64 receiver with pinned Opus 1.5.2, runs the full test suite including native Opus smoke coverage, validates third-party notices, and uploads a self-contained ZIP. | Current [manual-test preview](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161097/artifacts/10870335436) is a GitHub artifact archive of 68,185,044 bytes with digest `sha256:343b7acd06ce8f3a48783713bd1b8774883f2928eba760666654b1d363101169`; this digest covers the outer workflow artifact, not the ZIP extracted from it. It is an unsigned development preview, not a tagged release or physical Windows acceptance. |
| iOS | Simulator app builds on the current candidate; CI also runs shared PCM v1 packet-vector tests. Package contains `en-CA`, `en-US`, `ckb`, and `ku-Latn` UI catalogs plus localized microphone and local-network purpose strings. The app remains an experimental PCM v1 publisher. | [Current iOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161174) passed. The [simulator artifact](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161174/artifacts/10869596500) is simulator-only; no physical iPhone-to-receiver acceptance is recorded. |
| macOS | Hosted CI built the unsigned universal app and CoreAudio virtual-microphone driver, passed protocol tests and validated the exact uploaded archives. The validator checks app/driver bundle IDs, 0.1.6 version metadata, `en-CA`, both CPU architectures, and tester install/recovery files. | [Current macOS CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161353) passed. The [unsigned tester preview](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161353/artifacts/10869750677) remains build/protocol-test/package evidence, not driver installation or live routing acceptance. |
...
| Locale work | Android has `en-CA`, `en-US`, and preliminary `ckb`/`kmr` catalogs. Apple bundles verify the `en-CA` default, 38-string UI catalogs, distinct Sorani/Kurmanji resources, and localized system purpose strings. All 24 marketing HTML files declare source locale `en-CA`; core pages now offer a labelled US spelling preview. | The site preview only changes `licence`/`behaviour` spellings and is marked draft; Kurdish site translation and all other site locales remain absent. Apple en-US copy needs regional review; Kurdish drafts need fluent and script/layout review; Windows localization is unimplemented. Do not claim unreviewed locales as supported. See [`localization/README.md`](localization/README.md), [`localization/catalog-status.json`](localization/catalog-status.json), [`localization/apple/catalog-status.json`](localization/apple/catalog-status.json), and [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). |
| Website release-source truth | The selected cream-and-green homepage is integrated in canonical `dev/v3/`; its 16-route manifest, sitemap, local CTAs, current v0.1.5 download facts, privacy disclosures, and `en-CA` default are checked by `scripts/validate-site-release.mjs`. CI and Lighthouse target the same `dev/v3/` payload. | Local validator, browser review, [hosted site CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161097), and [Lighthouse](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161065) pass on the current candidate. Download the [website preview artifact](https://github.com/ryanspice/pocketmic-lan/actions/runs/36146161097/artifacts/10869835828) for manual review. Site copy remains English-only except the labelled US spelling preview. Kurdish/site translations, hands-on accessibility acceptance, deployment, live-page verification, and physical product routing remain separate gates. |
...
- The selected marketing-site replacement is integrated in `dev/v3/`; the current candidate keeps the existing canonical site URL, retained routes, and v0.1.5 published-download links. Manual review, release-asset reconciliation after tagging, and production deployment remain open gates.
...
- Website source boundary is resolved: `dev/v3/` is the canonical deployment payload, and both website validation and Lighthouse now target that directory. Keep the old `web/` demonstration surface out of release claims.
...
- The Android package migration is now stated in both the unreleased changelog and README: `com.canopydigital.pocketmic` installs separately from v0.1.5, and app data/pairing keys do not move automatically. Gate 2 manual acceptance still requires a clean-install/separate-install check on a device.
...
#### Gate 6 — Selected marketing-site staged acceptance

- The selected homepage from [Replace marketing site](codex://threads/01a0d5b9-83f5-7dc2-8fde-958d75ac661c) is integrated in the canonical `dev/v3/` source. Review the hosted CI preview artifact built from the candidate commit; do not treat this source integration as a live deployment.
- Retain the existing PocketMic LAN URL `https://canopydigital.ca/sites/pocketmic-lan/`. Inventory current routes before cutover; preserve working download, setup, privacy/policy, troubleshooting, roadmap, and release-note routes, or document and verify an explicit redirect/retirement map for each route.
- Reconcile every claim and link with the actual v0.1.6 outcome: repository and support links, exact tagged Android/Windows assets and checksums, signing state, version, codecs, VB-CABLE mapping, and accepted macOS preview status. Keep Linux and physical iOS distribution out of claims unless separately implemented and accepted. Do not reuse the supplied page's unrelated PocketMic links, 12 ms claim, or browser-as-transmitter assumptions.
- Keep `en-CA` as the default. Complete and fluently review every language offered by the replacement; until then, remove or clearly limit partial language controls and keep the existing labelled `en-US` spelling preview distinct from full translation support. Kurdish and other draft catalogs are not complete marketing translations.
- Run the integrated site from the exact hosted preview artifact and manually check desktop, tablet, and narrow phone layouts; light/dark behavior; keyboard and visible focus; no-JavaScript and reduced-motion fallbacks; every CTA/download/route; canonical and social metadata; and the browser Signal Lab's local-only purpose, permission explanation, denial, and unsupported-browser fallback.
- Preserve the prior deployed payload and record its file list/hashes before cutover. Prepare a versioned upload or other reviewable staging payload with a rollback path. Do not replace the public site from the detached candidate or an unverified build directory.

#### Gate 7 — Frozen-candidate CI, artifacts, and release decision

- Run every relevant workflow on one frozen candidate commit: Windows build/tests, Android build/lint/unit tests, iOS simulator build, macOS build/tests/package, website validation, and Lighthouse against the canonical `dev/v3/` release payload. CI must fail on unit-test failures.
...
- Only after Gates 1–7 pass: create the v0.1.6 tag and release and attach exact artifacts/checksums. Once those tagged assets resolve, deploy the approved site payload at the existing production path; verify the homepage, retained/redirected routes, downloads/checksums, policy links, metadata, and responsive behavior from outside the build environment. Roll back the site if any required check fails. Record the final commit, site payload hashes, deployment/rollback receipt, and post-publication checks in `BUILD-EVIDENCE.md`.

#### Gate 8 — Post-release product truth and staged promotion

- Verify README, release manifest, the replaced website/download/setup pages, and attached assets resolve to the published tag and exact artifact checksums. Confirm the published signing, codec, routing, and migration disclosures still match the build. Keep the previous site payload available until live verification passes.
- If Gate 5's demo/pilot criteria and Gate 6's site acceptance criteria passed, make at most two tailored public posts, one at a time: first to a relevant Android/audio/OBS tester audience, then to a maker/developer showcase. Recheck venue rules and format immediately before posting. Keep AI-development discussion separate and factual; defer a broader campaign until pilot results justify it.
...
| Site manual review, live verification, and deployment remain open | Review the hosted `dev/v3/` preview against the manual checklist; preserve the current production payload for rollback; deploy only after the tagged assets exist, then verify the public routes and downloads. | Blocks the v0.1.6 site cutover but not manual testing of the local/hosted candidate. |
...
- [Selected marketing-site task](codex://threads/01a0d5b9-83f5-7dc2-8fde-958d75ac661c) — approved visual direction and candidate history; now integrated locally, with manual review and deployment gates open.
- [`dev/V3-PLAN.md`](dev/V3-PLAN.md) — existing multi-page marketing source and deployment notes.
