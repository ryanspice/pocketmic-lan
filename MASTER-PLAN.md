...
| Windows, Android, web CI | Green on commit `c20696413510ef2b32e028f043198323ae6e9a31`; Android build/lint/JVM tests, Windows build/tests, and website release validation passed. | [Push CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36082802007) and [PR CI](https://github.com/ryanspice/pocketmic-lan/actions/runs/36082805512). Artifact names, digests, and expiry are recorded in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). |
| iOS | Simulator build passes on commit `c206964`; the app remains an experimental PCM v1 publisher. | [Push iOS CI run](https://github.com/ryanspice/pocketmic-lan/actions/runs/36082802002) uploaded a simulator app (artifact digest and expiry in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md)). No physical iPhone-to-receiver acceptance is recorded. |
| macOS | Hosted CI built the unsigned universal app and CoreAudio virtual-microphone driver, including the Opus mismatch warning, on commit `c206964`. | [Push macOS CI run](https://github.com/ryanspice/pocketmic-lan/actions/runs/36082801975) uploaded the updated physical-tester preview (artifact digest and expiry in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md)). This is build/package evidence only, not driver installation or live routing acceptance. |
...
The latest pushed source implementation commit is `ff690fcdcc94fca57e1a41b076bd087e5d0654ae` (`feat(android): expose Opus codec selection safely`). Push-triggered primary CI, iOS, and macOS workflows passed; associated pull-request CI, iOS, macOS, and Lighthouse runs passed as well. Android Packet C now has local JVM tests, a debug APK/native ABI build, lint, and hosted CI evidence. D4/D5 desktop changes are also hosted-CI green; remaining hands-on audio acceptance is still open. See the dated receipts in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). These are development-branch checks, not a frozen release-candidate rerun. Recheck the live workflows before tagging.
...
- **Desktop review and implementation handoff** ([review task](codex://threads/01a0d27f-4b70-78b3-a229-ff340cdea6d6); `DESKTOP-CONTROL-REVIEW.md`, `LUNA-IMPLEMENTATION-HANDOFF.md`): carry its findings into Gate 2. The review found buffer-control ownership, exact virtual-cable selection and Windows device-role restoration, live monitor switching, desktop DSP strength after phone Custom settings, and exporting diagnostics during an active session; D1-D5 implementation status and remaining hands-on checks are tracked in finding 8 and Gate 2 below. On the then-tested v0.1.5 phone, Auto-connect did not discover the receiver until the IP was manually changed. The user's required behavior is to clear stale endpoint/readiness values on either Auto-connect transition, restart LAN discovery when enabled, and complete a pre-capture authenticated handshake so the user can check receiver readiness before pressing Start. Preserve the pairing credential, never capture audio during discovery, and keep discovery, authentication/readiness, and actual audio delivery distinct. The Windows listener currently starts with the audio engine, so the idle responder and any versioned discovery/capability extension remain design and implementation work, with manual fallback retained. D4 and D5 code, local tests/builds, and hosted CI are complete; live Windows acceptance remains open for D1-D5, including the D5 Save-dialog retest. The September 24 phone session is historical v0.1.5-device evidence: 6,006 packets in one minute, no loss/rejections, 253 queue trims, and a prior pairing-key mismatch. The installed APK was not proven to come from this checkout; Opus use, audible destination-app routing, and measured end-to-end latency were not established. Do not count that session as v0.1.6 acceptance.
- **Readiness-first exposure review** ([review task](codex://threads/01a0cee2-483f-7d02-aa3c-17001dabdd20)): use product truth and the verified route as prerequisites; then capture one genuine 15–30 second end-to-end demo, run a roughly five-person Android/Windows pilot, fix the largest repeatable setup issue, and make at most two tailored posts one at a time (an Android/audio/OBS audience, then a maker/developer showcase). Recheck each venue's rules immediately before posting. Track reported successful routes and useful failure reports, not clicks or stars. Keep AI-development discussion separate from the user-facing product pitch. These are readiness gates, not a calendar-based campaign.
