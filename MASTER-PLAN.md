...
| Windows, Android, web CI | Green on the latest implementation commit `a584f1a`. | [Push CI run](https://github.com/ryanspice/pocketmic-lan/actions/runs/36076365312) includes Android build/lint/unit tests and Windows build/tests. |
| iOS | Simulator build passes on the latest implementation commit; the app remains an experimental PCM v1 publisher. | [Push iOS CI run](https://github.com/ryanspice/pocketmic-lan/actions/runs/36076365210). No physical iPhone-to-receiver acceptance is recorded. |
| macOS | Hosted CI builds unsigned universal app and CoreAudio virtual-microphone driver artifacts. | [Push macOS CI run](https://github.com/ryanspice/pocketmic-lan/actions/runs/36076365257). This is build/package evidence only, not driver installation or live routing acceptance. |
...
The latest source commit in this checkout is `a584f1a76d902114b3c63caf20663e579c44d801` (`fix(windows): apply desktop DSP preset overrides`). Its Windows core suite (162 tests) and desktop build (0 warnings/errors) passed locally. Push CI, iOS, and macOS workflows passed on this exact commit; the associated pull-request CI, iOS, macOS, and Lighthouse runs also passed. See the dated receipts in [`BUILD-EVIDENCE.md`](BUILD-EVIDENCE.md). These are development-branch checks, not a frozen release-candidate rerun. Recheck the live workflows before tagging.
...
- **Desktop review and implementation handoff** ([review task](codex://threads/01a0d27f-4b70-78b3-a229-ff340cdea6d6); `DESKTOP-CONTROL-REVIEW.md`, `LUNA-IMPLEMENTATION-HANDOFF.md`): carry its findings into Gate 2; D4 code is implemented with local tests/build evidence, while hosted CI and live Windows acceptance remain open, and D5 still needs implementation. The September 24 phone session is historical v0.1.5-device evidence: 6,006 packets in one minute, no loss/rejections, 253 queue trims, and a prior pairing-key mismatch. The installed APK was not proven to come from this checkout; Opus use, audible destination-app routing, and measured end-to-end latency were not established. Do not count that session as v0.1.6 acceptance.
...
- Complete remaining handoff packets for clock-drift wrap correctness, deliberate DSP preset ownership, live monitor-device changes, and export during an active session. D1's automatic/manual buffer mode and effective-target display are implemented, and D4's exact cable mapping and role-safe default-switch transaction now have local tests/build evidence. D1-D4 still need live Windows UI/audio acceptance; D5 active-session export remains to implement. Add bounded tests around failure, rollback, and concurrency behavior.
